...
__global__ void gpu_eh(const int size, const int x, const floatT t, const floatT sigma,
	...
	if (i>0 && j>0) {
		s_e[threadIdx.x][threadIdx.y] += (hy[INDX(i,j,size-1)]-hy[INDX(i-1,j,size-1)])- (hx[INDX(i,j,size)]-hx[INDX(i,j-1,size)]);
		if (threadIdx.x==(THREADS_PER_BLOCK-1)) {
			s_e[THREADS_PER_BLOCK][threadIdx.y] += (hy[INDX(i+1,j,size-1)]-hy[INDX(i,j,size-1)])- (hx[INDX(i+1,j,size)]-hx[INDX(i+1,j-1,size)]);
		}			
		if (threadIdx.y==(THREADS_PER_BLOCK-1)) {
			s_e[threadIdx.x][THREADS_PER_BLOCK] += (hy[INDX(i,j+1,size-1)]-hy[INDX(i-1,j+1,size-1)])- (hx[INDX(i,j+1,size)]-hx[INDX(i,j,size)]);
		}		
	...
	if (i>0) {
		hx[INDX(i,j,size)] -= 0.5*(s_e[threadIdx.x][threadIdx.y+1] - s_e[threadIdx.x][threadIdx.y]);
	}
	if (j>0) {
		hy[INDX(i,j,size-1)] += 0.5*(s_e[threadIdx.x+1][threadIdx.y] - s_e[threadIdx.x][threadIdx.y]);
	}
}
